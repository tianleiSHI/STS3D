...
def bbox_iou_3d(box1, box2, xyzwhd=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    ...
    # Get the coordinates of bounding boxes
    if xyzwhd:  # transform from xywh to xyxy
        # (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
        # w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
        # b1_x1, b1_x2, b1_y1, b1_y2 = x1 - w1_, x1 + w1_, y1 - h1_, y1 + h1_
        # b2_x1, b2_x2, b2_y1, b2_y2 = x2 - w2_, x2 + w2_, y2 - h2_, y2 + h2_

        (x1, y1, z1, w1, h1, d1), (x2, y2, z2, w2, h2, d2) = box1.chunk(6, -1), box2.chunk(6, -1)
        w1_, h1_, d1_, w2_, h2_, d2_ = w1 / 2, h1 / 2, d1 / 2, w2 / 2, h2 / 2, d2 / 2
        b1_x1, b1_x2, b1_y1, b1_y2, b1_z1, b1_z2 = x1 - w1_, x1 + w1_, y1 - h1_, y1 + h1_, z1 - d1_, z1 + d1_
        b2_x1, b2_x2, b2_y1, b2_y2, b2_z1, b2_z2 = x2 - w2_, x2 + w2_, y2 - h2_, y2 + h2_, z2 - d2_, z2 + d2_
    else:  # x1, y1, x2, y2 = box1
        b1_x1, b1_y1, b1_z1, b1_x2, b1_y2, b1_z2 = box1.chunk(6, -1)
        b2_x1, b2_y1, b2_z1, b2_x2, b2_y2, b2_z2 = box2.chunk(6, -1)
        w1, h1, d1 = b1_x2 - b1_x1, b1_y2 - b1_y1, b1_z2 - b1_z1 + eps
        w2, h2, d2 = b2_x2 - b2_x1, b2_y2 - b2_y1, b2_z2 - b2_z1 + eps
    ...
    if CIoU or DIoU or GIoU:
        ...
        cd = torch.max(b1_z2, b2_z2) - torch.min(b1_z1, b2_z1)  # convex height
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = cw ** 2 + ch ** 2 + cd ** 2 + eps  # convex diagonal squared
            rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2 + \
                    (b2_z1 + b2_z2 - b1_z1 - b1_z2) ** 2) / 8  # center dist ** 2
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                return iou - (rho2 / c2 + v * alpha)  # CIoU
            return iou - rho2 / c2  # DIoU
        c_area = cw * ch * cd + eps  # convex area
        return iou - (c_area - union) / c_area  # GIoU https://arxiv.org/pdf/1902.09630.pdf
    return iou  # IoU
