...
def non_max_suppression_3d(
    ...
    if mps:  # MPS not fully supported yet, convert tensors to CPU before NMS
        prediction = prediction.cpu()
    bs = bboxes.shape[0]  # batch size

    xc = bboxes[scores > conf_thres]  # candidates
...
def crop_mask(masks, boxes, loose_factor):
    ...
    """
    n, w, h, d = masks.shape
    x1, y1, z1, x2, y2, z2 = torch.chunk(boxes[:, :, None, None], 6, 1)  # x1 shape(1,1,n)
    if loose_factor is not None:
        pw, ph, pd = x2 - x1, y2 - y1, z2 - z1
        x1 = x1 - pw * loose_factor
        y1 = y1 - ph * loose_factor
        z1 = z1 - pd * loose_factor
        x2 = x2 + pw * loose_factor
        y2 = y2 + ph * loose_factor
        z2 = z2 + pd * loose_factor

    x = torch.arange(w, device=masks.device, dtype=x1.dtype)[None, :, None, None]  # rows shape(w,1,1)
    y = torch.arange(h, device=masks.device, dtype=x1.dtype)[None, None, :, None]  # cols shape(1,h,1)
    z = torch.arange(d, device=masks.device, dtype=x1.dtype)[None, None, None, :]  # cols shape(1,1,d)
    return masks * ((x >= x1) * (x < x2) * (y >= y1) * (y < y2) * (z >= z1) * (z < z2))


def loose_mask(masks, boxes, loose_factor, min_area=30):
    """
    It takes a mask and a bounding box, and returns a mask that is cropped to the bounding box
    ...
    x1, y1, z1, x2, y2, z2 = torch.chunk(boxes[:, :, None, None], 6, 1)  # x1 shape(1,1,n)
    if loose_factor is not None:
        pw = (x2 - x1) * loose_factor
        ph = (y2 - y1) * loose_factor
        pd = (z2 - z1) * loose_factor
        pw = torch.max(pw, torch.ones_like(pw) * min_area)
        ph = torch.max(ph, torch.ones_like(ph) * min_area)
        pd = torch.max(pd, torch.ones_like(pd) * min_area)
...
def process_mask_upsample(protos, masks_in, bboxes, shape, loose_factor=None):
    """
...
def process_mask(protos, masks_in, bboxes, shape, upsample=False, loose_factor=None):
    """
    ...
    c, mw, mh, md = protos.shape  # CHW
    iw, ih, id = shape
    masks = (masks_in @ protos.float().view(c, -1)).view(-1, mw, mh, md)  # CHW

    downsampled_bboxes = bboxes.clone()
    downsampled_bboxes[:, 0] *= mw / iw
    downsampled_bboxes[:, 1] *= mh / ih
    downsampled_bboxes[:, 2] *= md / id
    downsampled_bboxes[:, 3] *= mw / iw
    downsampled_bboxes[:, 4] *= mh / ih
    downsampled_bboxes[:, 5] *= md / id

    masks = crop_mask(masks, downsampled_bboxes, loose_factor)  # CHW
    if upsample:
        masks = F.interpolate(masks[None], shape, mode='trilinear', align_corners=False)[0]  # CHW
    masks = masks.sigmoid()
    return masks.gt_(0.5)
