...
from .metrics import bbox_iou, bbox_iou_3d
...
def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9):
    ...
    lt, rb = gt_bboxes.view(-1, 1, 6).chunk(2, 2)  # left-top, right-bottom
...
class TaskAlignedAssigner(nn.Module):
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        overlaps[mask_gt] = bbox_iou_3d(gt_boxes, pd_boxes, xyzwhd=False, DIoU=True).squeeze(-1).clamp(0)

        align_metric = bbox_scores.pow(self.alpha) * overlaps.pow(self.beta)
        # align_metric = overlaps.pow(self.beta)
...
def make_anchors(feats, strides, grid_cell_offset: float = 0.5):
    ...
    for i in range(3):
        ...
        # anchor_points.append(torch.stack((sd, sw, sh), -1).view(-1, 3))
        anchor_points.append(torch.stack((sh, sw, sd), -1).view(-1, 3))
...
def dist2bbox(distance, anchor_points, xywh: bool = True, dim: int = -1):
    ...
    ltn, rbf = distance.chunk(2, dim)
    x1y1z1 = anchor_points - ltn
    x2y2z2 = anchor_points + rbf

    if xywh:
        c_xyz = (x1y1z1 + x2y2z2) / 2
        whd = x2y2z2 - x1y1z1
        return torch.cat((c_xyz, whd), dim)  # xywh bbox
